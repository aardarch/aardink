...
- **LSP Editor UI Components**: Floating `SignatureHelpPopup` for parameter hints, `CodeActionMenu` popup for quick fixes/refactorings, "💡 Quick Fix" button in `AnnotationTooltip`, and `RenameDialog` prompt for symbol renaming. A rename whose edits arrive after the document has changed is discarded rather than applied to offsets that no longer mean what the server measured. The tooltip's dismiss and quick-fix controls are real buttons with accessibility labels and minimum touch targets (as are the find panel's arrow and close buttons), the rename dialog's text field is labelled, and the signature-help popup is taken down when the host detaches the language service.
